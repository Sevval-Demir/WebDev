...
# Video dosyasını aç
video_path = "Bad4.mp4"  # Video dosyasının yolu
...
while cap.isOpened() and frame_count < 60:  # Toplam 60 kare işle
    ret, frame = cap.read()
    if not ret:
        break
...
def fill_missing_data(coords):
    ...
    for i in range(len(coords)):
        if coords[i] is None:  # Eğer eksik değer varsa
            prev_val = coords[i - 1] if i > 0 else None
            next_val = coords[i + 1] if i < len(coords) - 1 else None
...
# Grafik çizimi
frames = list(range(1, 61))  # 1'den 60'a kadar kareler

# Her parmak için ayrı grafik çiz
for i, finger_name in enumerate(["Thumb", "Index", "Middle", "Ring", "Pinky"]):
    plt.figure()
    plt.plot(frames, x_coords[i], label=f"{finger_name} X-Coordinate")
    plt.plot(frames, y_coords[i], label=f"{finger_name} Y-Coordinate", linestyle="dashed")
